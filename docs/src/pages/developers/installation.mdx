...
Workers in Aleph (the `aleph-worker` as well as `ingest-file`) are based off common code in [servicelayer](https://github.com/alephdata/servicelayer). These workers are able to run in threads, but for the purposes of better scaling and more predictable performance Aleph's defaults disable threading by setting the `WORKER_THREADS` setting to 0, thus running one worker per process. This way one would scale the amount of necessary workers by spinning up more of the respective container.
