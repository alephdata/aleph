...
def upgrade():
    ...
    op.create_index(
        op.f("ix_document_tag_document_id"),
        "document_tag",
        ["document_id"],
        unique=False,
    )  # noqa
    op.create_index(
        op.f("ix_document_tag_key"), "document_tag", ["key"], unique=False
    )  # noqa
    op.create_index(
        op.f("ix_document_tag_origin"), "document_tag", ["origin"], unique=False
    )  # noqa
    op.drop_column("cache", "created_at")
    op.alter_column(
        "document", "content_hash", existing_type=sa.VARCHAR(length=65), nullable=True
    )
