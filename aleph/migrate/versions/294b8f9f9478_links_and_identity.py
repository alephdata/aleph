...
def upgrade():
    ...
    op.create_index(
        op.f("ix_link_collection_id"), "link", ["collection_id"], unique=False
    )
    ...
    op.add_column(
        "entity_identity", sa.Column("match_id", sa.String(length=254), nullable=False)
    )
    op.add_column("entity_identity", sa.Column("judge_id", sa.Integer(), nullable=True))
    op.add_column(
        "entity_identity", sa.Column("judgement", sa.Integer(), nullable=False)
    )
    op.create_index(
        op.f("ix_entity_identity_match_id"),
        "entity_identity",
        ["match_id"],
        unique=False,
    )
