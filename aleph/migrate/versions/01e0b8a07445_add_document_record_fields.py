...
def upgrade():
    ...
    op.alter_column(
        "document_record", "sheet", existing_type=sa.INTEGER(), nullable=True
    )
    op.alter_column(
        "document_record", "row_id", existing_type=sa.INTEGER(), nullable=True
    )
    op.create_index(
        op.f("ix_document_record_document_id"),
        "document_record",
        ["document_id"],
        unique=False,
    )
    ...
    q = sa.insert(records).from_select(
        [records.c.index, records.c.text, records.c.document_id], q
    )
    bind.execute(q)
