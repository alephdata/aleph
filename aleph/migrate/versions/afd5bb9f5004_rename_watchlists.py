...
def upgrade():
    ...
    op.execute(
        "ALTER TABLE permission ALTER COLUMN resource_type TYPE perm_type_coll"
        " USING resource_type::text::perm_type_coll;"
    )
    op.alter_column(
        "permission", "resource_type", type_=tmp_type, existing_type=old_type
    )
    meta = sa.MetaData()
    meta.bind = bind
    meta.reflect()
    ...
    op.execute(
        "ALTER TABLE permission ALTER COLUMN resource_type TYPE permission_type"
        " USING resource_type::text::permission_type;"
    )
    tmp_type.drop(bind, checkfirst=False)
    op.create_foreign_key(
        "collection_creator_id_fkey", "collection", "role", ["creator_id"], ["id"]
    )
    op.create_foreign_key(
