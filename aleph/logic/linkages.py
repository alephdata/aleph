...
def decide_xref(xref, decision=None, context_id=None, decider_id=None):
    ...
    entity_linkage = Linkage.by_entity(
        entity_id, decision=True, collection_id=collection_id, context_id=context_id
    ).first()
    match_id = xref.get("match_id")
    match_collection_id = xref.get("match_collection_id")
    match_linkage = Linkage.by_entity(
        match_id,
        decision=True,
